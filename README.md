...
```
Busca a spec em `docs/specs/1234-spec.md` se o arquivo já existir localmente (fluxo do `/specforge-create-spec`); senão, busca pela task `spec - {nome do projeto}` no card 1234 via MCP (fluxo do `/specforge-analyzer`, modo `task` — nesse caso o arquivo local só é gravado depois, junto do commit, nunca antes). Apresenta um plano de implementação (incluindo a branch que vai usar) e aguarda confirmação. **Nunca implementa direto em `main`/`master`**: cria (ou reutiliza) a branch `specforge/1234` a partir da branch atual antes de tocar em qualquer arquivo. Em seguida executa as mudanças, roda testes unitários, verifica coerência com as regras de negócio, commita (incluindo a spec, se veio da task) e faz push **dessa branch** (`specforge/1234`) para o remoto, e publica o changelog no card de origem. Abrir o PR de `specforge/1234` para a branch principal continua manual. Se um card afetou mais de um projeto, rode este comando separadamente dentro de cada um — cada projeto tem sua própria task de spec, então devs diferentes podem rodar isso em paralelo em projetos diferentes do mesmo card.
