...
```
Antes de tudo, confirma via MCP que o card 1234 existe de verdade no tracker configurado — **o MCP é obrigatório para este comando, sem exceção, mesmo quando a spec já existe localmente** (fluxo `/specforge-create-spec`): sem consultar o tracker não há como garantir que a branch fica vinculada a um card real, e criá-la sem essa confirmação não é seguro. Se não encontrar, pergunta no console qual ID correto usar (ou cancela); sem essa confirmação não cria branch nem implementa nada. Depois busca a spec em `docs/specs/1234-spec.md` se o arquivo já existir localmente; senão, busca pela task `spec - {nome do projeto}` no card 1234 (fluxo do `/specforge-analyzer`, modo `task` — nesse caso o arquivo local só é gravado depois, junto do commit, nunca antes). Apresenta um plano de implementação (incluindo a branch que vai usar) e aguarda confirmação. **Nunca implementa direto em `main`/`master`**: cria (ou reutiliza) a branch `specforge/1234` a partir da branch atual antes de tocar em qualquer arquivo. Em seguida executa as mudanças, roda testes unitários, verifica coerência com as regras de negócio, commita (incluindo a spec, se veio da task) e faz push **dessa branch** (`specforge/1234`) para o remoto. Grava um único arquivo local, `docs/changelogs/1234.md`, sempre no mesmo template, com duas seções: o **changelog** técnico e as **evidências de aceite** (em linguagem simples para o QA — resumo do que foi entregue, evidência e passo a passo de reprodução por critério de aceite com dados reais dos testes, e a cobertura obtida). Publica o **arquivo completo** (nunca uma seção isolada) como comentário: sempre no card de origem, e também na mesma task de spec quando a spec veio do fluxo `/specforge-analyzer`. Abrir o PR de `specforge/1234` para a branch principal continua manual. Se um card afetou mais de um projeto, rode este comando separadamente dentro de cada um — cada projeto tem sua própria task de spec, então devs diferentes podem rodar isso em paralelo em projetos diferentes do mesmo card.
